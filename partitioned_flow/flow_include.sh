...
function countall {
  ...
  sumlogs msg_total $LOGDIR/report_tsarra_f20_*.log
  totsarra="${tot}"
  ...
  if [ "${sarra_py_version:0:1}" == "3" ]; then
       ...
       #countthem "`grep -a after_work\ rejected  "$LOGDIR"/sarra_download_f20_*.log | grep -v DEBUG | wc -l`"
       countthem "`grep -a 'rejected: 304 Not modified' "$LOGDIR"/sarra_download_f20_*.log | wc -l`"
       totwinnowed="${tot}"
  else
       ...
       countthem "`grep -a rejected  "$LOGDIR"/sarra_download_f20_*.log | grep -v DEBUG | wc -l`"
       totwinnowed="${tot}"
  fi
  ...
  if [[ "${sarra_py_version}" > "3.00.52" ]]; then
      countthem "`grep -a 'after_post posted .* a directory with' ${LOGDIR}/watch_f40_*.log | wc -l`"
      totdirwatch="${tot}"
  ...
  sumlogs msg_total $LOGDIR/subscribe_amqp_f30_*.log
  totmsgamqp="${tot}"

  if [ "${sarra_py_version:0:1}" == "3" ]; then
	  countthem "`grep -aE 'after_work (directory|downloaded) ok:' "$LOGDIR"/subscribe_amqp_f30_*.log | wc -l`"
  else
      countthem "`grep -a '\[INFO\] file_log downloaded to:' "$LOGDIR"/subscribe_amqp_f30_*.log | wc -l`"
  fi
  totfileamqp="${tot}"

  if [[ "${sarra_py_version}" > "3.00.25" ]]; then
      countthem "`grep -a 'log after_post posted' "$LOGDIR"/sender_tsource2send_f50_*.log | grep \'m, | wc -l`"
      totdirsent="${tot}"
      countthem "`grep -a 'log after_post posted' "$LOGDIR"/sender_tsource2send_f50_*.log | grep directory | wc -l`"
  ...
  if [ "${sarra_py_version:0:1}" == "3" ]; then
      countthem "`grep -a 'log after_post posted' "$LOGDIR"/sender_tsource2send_f50_*.log | grep -v directory | grep -v \'m, | wc -l`"
  else
      countthem "`grep -a '\[INFO\] post_log notice' "$LOGDIR"/sender_tsource2send_f50_*.log | wc -l`"
  fi
  totfilesent="${tot}"
  ...
  if [ "${sarra_py_version:0:1}" == "3" ]; then
      ...
      countthem "`grep -aE "$all_events" "$LOGDIR"/subscribe_rabbitmqtt_f31_*.log | grep -v DEBUG | wc -l`"
  else
  ...
  countthem "`grep -aE "directory\ ok:" "$LOGDIR"/subscribe_rabbitmqtt_f31_*.log | grep -v DEBUG | wc -l`"
  totsubmqttDir="${tot}"

  
  countthem "`grep -aE "$all_events" "$LOGDIR"/subscribe_u_sftp_f60_*.log | grep -v DEBUG | wc -l`"
  totsubu="${tot}"
  countthem "`grep -aE "$all_events" "$LOGDIR"/subscribe_cp_f61_*.log | grep -v DEBUG | wc -l`"
  totsubcp="${tot}"
  if [ "${sarra_py_version:0:1}" == "3" ]; then
      all_events="directory ok|downloaded ok"
      countthem "`grep -aE "$all_events" "$LOGDIR"/subscribe_ftp_f70_*.log | grep -v DEBUG | wc -l`"
  else
      countthem "`grep -aE "$no_hardlink_events" "$LOGDIR"/subscribe_ftp_f70_*.log | grep -v DEBUG | wc -l`"
  fi
  totsubftp="${tot}"
  countthem "`grep -aE "$all_events" "$LOGDIR"/subscribe_q_f71_*.log | grep -v DEBUG | wc -l`"
  totsubq="${tot}"

  if [ "${sarra_py_version:0:1}" == "3" ]; then
       countthem "`grep -a 'log after_post posted' "$LOGDIR"/poll_sftp_f62_*.log | wc -l`"
       totpoll1="${tot}"
       totpoll_mirrored="`grep -a ', now saved' "$LOGDIR"/poll_sftp_f63_*.log | awk ' { print $18 } '|tail -1`"
  else
       countthem "`grep -a '\[INFO\] post_log notice' "$LOGDIR"/poll_sftp_f62_*.log | wc -l`"
       totpoll1="${tot}"
  fi
  ...
  #countthem "`grep -a '\[INFO\] post_log notice' "$LOGDIR"/sarra_download_f20_*.log | wc -l`"
  if [ "${sarra_py_version:0:1}" == "3" ]; then
      countthem "`grep -a 'after_accept accepted:' "$LOGDIR"/sarra_download_f20_*.log | wc -l`"
      totsarx="${tot}"
      countthem "`grep -a 'rejected: 304 Not modified' "$LOGDIR"/sarra_download_f20_*.log | wc -l`"
      totsarx=$((${totsarx}+${tot}))
      ...
      countthem "`grep -a '\[INFO\] post_log notice' "$LOGDIR"/sarra_download_f20_*.log | wc -l`"
      totsarp="${tot}"
      countthem "`grep -a 'received' "$LOGDIR"/sarra_download_f20_*.log | wc -l`"
      totsarx="${tot}"
  fi
  ...
  if [[ "${sarra_c_version}" > "3.22.51" ]]; then

     countthem "`grep -a '\[INFO\] cpost published:' $LOGDIR/cpost_pelle_dd1_f04_*.log | wc -l`"
     totcpelle04p="${tot}"

     countthem "`grep -a '\[INFO\] cpost published:' $LOGDIR/cpost_pelle_dd2_f05_*.log | wc -l`"
     totcpelle05p="${tot}"

     countthem "`grep -a '\[INFO\] cpump published:' $LOGDIR/cpump_xvan_f14_*.log | grep -v \"directory\" | wc -l`"
     totcvan14p="${tot}"

     countthem "`grep -a '\[INFO\] cpump published:' $LOGDIR/cpump_xvan_f15_*.log | grep -v \"directory\" | wc -l`"
     totcvan15p="${tot}"

     countthem "`grep -a '\[INFO\] cpost published:' $LOGDIR/cpost_veille_f34_*.log | grep -v \"directory\" | grep -v '\"size\":\"0\"' | awk '{ print $8 }' | wc -l`"
     totcveille="${tot}"
  else
     countthem "`grep -a '\[INFO\] published:' $LOGDIR/cpost_pelle_dd1_f04_*.log | wc -l`"
     totcpelle04p="${tot}"

     countthem "`grep -a '\[INFO\] published:' $LOGDIR/cpost_pelle_dd2_f05_*.log | wc -l`"
     totcpelle05p="${tot}"

     countthem "`grep -a '\[INFO\] published:' $LOGDIR/cpump_xvan_f14_*.log | grep -v \"directory\" | wc -l`"
     totcvan14p="${tot}"

     countthem "`grep -a '\[INFO\] published:' $LOGDIR/cpump_xvan_f15_*.log | grep -v \"directory\" | wc -l`"
     totcvan15p="${tot}"

     countthem "`grep -a '\[INFO\] published:' $LOGDIR/cpost_veille_f34_*.log | grep -v \"directory\" | grep -v '\"size\":\"0\"' | awk '{ print $8 }' | wc -l`"
     totcveille="${tot}"
  fi


  if [ "${sarra_py_version:0:1}" == "3" ]; then
      countthem "`grep -aE 'after_work downloaded ok' $LOGDIR/subscribe_cdnld_f21_*.log | wc -l`"
  else
      countthem "`grep -a '\[INFO\] file_log downloaded ' $LOGDIR/subscribe_cdnld_f21_*.log | wc -l`"
  fi
  totcdnld="${tot}"
  ...
  if [ "${sarra_py_version:0:1}" == "3" ]; then
      ...
      zerobytefilesxferred="`grep 0--1 $LOGDIR/subscribe_cfile_f44_*.log | wc -l`"
      tot=$((${tot}-${zerobytefilesxferred}))
  else
      countthem "`grep -a '\[INFO\] file_log downloaded ' $LOGDIR/subscribe_cfile_f44_*.log | wc -l`"
  fi
  totcfile="${tot}"

  if [[ $(ls "$LOGDIR"/shovel_pclean_f90*.log 2>/dev/null) ]]; then
      countthem "`grep -a '\[INFO\] post_log notice' "$LOGDIR"/shovel_pclean_f90*.log | wc -l`"
      totpropagated="${tot}"
  else
      totpropagated="0"
  fi

  if [[ $(ls "$LOGDIR"/shovel_pclean_f92*.log 2>/dev/null) ]]; then
      countthem "`grep -a '\[INFO\] post_log notice' "$LOGDIR"/shovel_pclean_f92*.log | wc -l`"
      totremoved="${tot}"
  else
      totremoved="0"
  fi
  ...
  awk 'BEGIN { lr=0; }; /msg_log received/ { lr++; print lr, FILENAME, $0 ; next; }; { lr=0; } '  $LOGDIR/subscribe_*_f??_??.log  | grep -v '^1 ' >$missedreport
