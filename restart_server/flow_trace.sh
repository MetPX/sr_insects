...
grep "$1"  "$LOGDIR"/"${LGPFX}"shovel_t_dd1_f00_01.log
grep "$1"  "$LOGDIR"/"${LGPFX}"shovel_t_dd2_f00_01.log

#printf "\n\nFLOW f1x winnowing\n\n"
#grep $1  $LOGDIR/${LGPFX}winnow*

printf "\n\nFLOW f2x download to local mirror\n\n"
grep "$1"  "$LOGDIR"/"${LGPFX}"sarra_* 

printf "\n\nFLOW f3x downloading from http://localhost to downloaded_by_sub_amqp f3x\n\n"
grep "$1"  "$LOGDIR"/"${LGPFX}"subscribe_amqp_f30_*.log


printf "\n\nFLOW f4x watch\n\n"
grep "$1"  "$LOGDIR"/"${LGPFX}"watch_f40_01.log

printf "\n\nFLOW f60 subscribe & sftp://localhost download to downloaded_by_sub_u\n\n"
grep "$1"  "$LOGDIR"/"${LGPFX}"subscribe_u_sftp_f60_01.log
grep "$1"  "$LOGDIR"/"${LGPFX}"subscribe_u_sftp_f60_0?.log
...
grep "$1"  "$LOGDIR"/"${LGPFX}"subscribe_q_f71_*.log
