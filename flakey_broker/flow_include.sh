...
function countall {
  ...
  if [ "${sarra_py_version:0:1}" == "3" ]; then
       countthem "`grep 'log after_post posted' "$LOGDIR"/${LGPFX}watch_f40_*.log | wc -l`"
  else
  ...
  if [ "${sarra_py_version:0:1}" == "3" ]; then
      countthem "`grep -aE "do_download downloaded ok|written from message ok" "$LOGDIR"/${LGPFX}subscribe_q_f71_*.log | wc -l`"
  else
