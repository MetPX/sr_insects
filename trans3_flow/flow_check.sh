...
calcres "${totsarx}" "${tot2stat}" "${LGPFX}sarra\t (${totsarx}) should receive the same number of items as both static file trees (${tot2stat})"
calcres "${totsarp}" "${staticfilecount}" "${LGPFX}sarra\t (${totsarp}) should publish the same number of items as the data directory\t (${staticfilecount})"
calcres "${totrejected}" "${staticfilecount}" "${LGPFX}sarra\t (${totrejected}) should reject the same number of items as static tree\t (${staticfilecount})"
