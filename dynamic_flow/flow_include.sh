...
function countall {
  ...
  countthem "`grep -a truncating "$LOGDIR"/sr_sarra_download_f20_*.log | grep -a -v DEBUG | wc -l`"
  totshortened="${tot}"
  ...
  countthem "`grep -a -E "$all_events" "$LOGDIR"/sr_subscribe_rabbitmqtt_f31_*.log | grep -a -v DEBUG | wc -l`"
  totsubrmqtt="${tot}"
  countthem "`grep -a -E "$all_events" "$LOGDIR"/sr_subscribe_u_sftp_f60_*.log | grep -a -v DEBUG | wc -l`"
  totsubu="${tot}"
  countthem "`grep -a -E "$all_events" "$LOGDIR"/sr_subscribe_cp_f61_*.log | grep -a -v DEBUG | wc -l`"
  totsubcp="${tot}"
  countthem "`grep -a -E "$no_hardlink_events" "$LOGDIR"/sr_subscribe_ftp_f70_*.log | grep -a -v DEBUG | wc -l`"
  totsubftp="${tot}"
  countthem "`grep -a -E "$all_events" "$LOGDIR"/sr_subscribe_q_f71_*.log | grep -a -v DEBUG | wc -l`"
  totsubq="${tot}"

  countthem "`grep -a '\[INFO\] post_log notice' "$LOGDIR"/sr_poll_f62_*.log | wc -l`"
  totpoll1="${tot}"

  countthem "`grep -a '\[INFO\] post_log notice' $srposterlog | grep -a -v shim | wc -l`"
  totpost1="${tot}"

  countthem "`grep -a '\[INFO\] published:' $srposterlog | grep -a shim | wc -l`"
  totshimpost1="${tot}"
  ...
  awk 'BEGIN { lr=0; }; /msg_log received/ { lr++; print lr, FILENAME, $0 ; next; }; { lr=0; } '  $LOGDIR/sr_subscribe_*_f??_??.log  | grep -a -v '^1 ' >$missedreport
