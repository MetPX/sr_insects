...
flow_configs="audit/ $(cd "$CONFDIR" || exit; ls */*f[0-9][0-9].conf 2>/dev/null; ls poll/pulse.conf 2>/dev/null)"
...
flow_configs="$(cd "${SR_TEST_CONFIGS}" || exit; ls */*f[0-9][0-9].conf 2>/dev/null; ls */*f[0-9][0-9].inc 2>/dev/null; ls poll/pulse.conf 2>/dev/null)"
flow_configs="$(echo "${flow_configs}" | tr '\n' ' ')"
...
if [ "${sarra_py_version:0:1}" == "3" ]; then
    echo "$flow_configs" |  sed 's/ / ;\n rm -f /g' | sed '1 s|^| rm -f |' | sed '/^ rm -f post/d' | sed 's+/+_+g' | sed '/conf[ ;]*$/!d' | sed 's/\.conf/_[0-9][0-9].log\*/g' | (cd "$LOGDIR" || exit; sh )

else
    echo "$flow_configs" |  sed 's/ / ;\n rm -f sr_/g' | sed '1 s|^| rm -f sr_|' | sed '/^ rm -f sr_post/d' | sed 's+/+_+g' | sed '/conf[ ;]*$/!d' | sed 's/\.conf/_[0-9][0-9].log\*/g' | (cd "$LOGDIR" || exit; sh )
fi
