...
class SrConfigChecksumTestCase(SrConfigTestCase):
    ...
    def test_checksum_d(self):
        ...
        with open(self.tmppath, 'rb') as f:
            for i in range(4):
                chunk = f.read(10)
                chkd.update(chunk)
        self.assertEqual(chkd.get_value(), '7efaff9e615737b379a45646c755c492', "test 02c: checksum_d did not work")
    ...
    def test_checksum_N(self):
        ...
        self.assertEqual(chk_n.get_value(), long_chksum, "test 02e: checksum_N did not work")
...
class SrConfigPluginScriptTestCase(SrConfigTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.ok, self.path = self.cfg.config_path("plugins", "scrpt.py", mandatory=True, ctype='py')
    ...
    def test_find_script(self):
        self.assertTrue(self.ok, "test 03: problem with config_path script not found")
...
class SrConfigGeneralTestCase(SrConfigTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.cfg.args(['-expire', '3h', '-message_ttl', '3W', '--randomize', '--assemble', 'True', '-logrotate', '5m',
    ...
    def test_lr_interval(self):
        self.assertEqual(self.cfg.lr_interval, 1, "test 08a: args problem logrotate %s" % self.cfg.lr_interval)

    def test_lr_backupCount(self):
        self.assertEqual(self.cfg.lr_backupCount, 5, "test 08b: args problem logrotate %s" % self.cfg.lr_backupCount)

    def test_lr_when(self):
        self.assertEqual(self.cfg.lr_when, 'm', "test 08c: args problem logrotate %s" % self.cfg.lr_when)

    def test_expire(self):
        self.assertEqual(self.cfg.expire, self.expire_value, "test 09: args problem expire %s" % self.cfg.expire)

    def test_msg_ttl(self):
        self.assertEqual(self.cfg.message_ttl, self.message_value,
    ...
    def test_broker_url(self):
        ...
        self.assertEqual(self.cfg.broker.geturl(), "amqp://a:b@toto", "test 12: varsub problem with replacing HOSTNAME")

    def test_partflg(self):
        opt1 = "parts i,128"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.partflg, 'i', "test 13a: option parts or module validate_parts")
        self.assertEqual(self.cfg.blocksize, 128, "test 13b: option parts or module validate_parts")

    def test_sumflg_zd(self):
        opt1 = "sum z,d"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.sumflg, 'z,d', "test 14: option sum or module validate_sum")

    def test_sumflg_r0(self):
        opt1 = "sum R,0"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.sumflg, 'R,0', "test 15: option sum or module validate_sum")

    def test_sumflg_d(self):
        opt1 = "sum d"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.sumflg, 'd', "test 16a: option sum or module validate_sum")
        self.assertEqual(self.cfg.sumalgo.registered_as(), 'd', "test 16b: option sum or module validate_sum")

    def test_sum_0(self):
        opt1 = "sum 0"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.sumflg, '0', "test 17a: option sum or module validate_sum")
        self.assertEqual(self.cfg.sumalgo.registered_as(), '0', "test 17b: option sum or module validate_sum")

    def test_sum_n(self):
        opt1 = "sum n"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.sumflg, 'n', "test 18a: option sum or module validate_sum")
        self.assertEqual(self.cfg.sumalgo.registered_as(), 'n', "test 18b: option sum or module validate_sum")
    ...
    def test_sum_N(self):
        opt1 = "sum N"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.sumflg, 'N', "test 19c: option sum or module validate_sum")
        self.assertEqual(self.cfg.sumalgo.registered_as(), 'N', "test 19d: option sum or module validate_sum")

    def test_movepath(self):
        opt1 = "move toto titi"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.movepath[0], 'toto', "test 20a: option move for sr_post does not work")
        self.assertEqual(self.cfg.movepath[1], 'titi', "test 20b: option move for sr_post does not work")
    ...
    def test_postpath(self):
        ...
        opt1 = "path .. ."
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.postpath[0], os.path.abspath('..'), "test 21a: option path for sr_post does not work")
        self.assertEqual(self.cfg.postpath[1], os.path.abspath('.'), "test 21b: option path for sr_post does not work")

    def test_inflight(self):
        opt1 = "inflight ."
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.inflight, '.', "test 22: option inflight . does not work")

    def test_inflight_tmp(self):
        opt1 = "inflight .tmp"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.inflight, '.tmp', "test 23: option inflight .tmp does not work")

    def test_inflight_1_5(self):
        opt1 = "inflight 1.5"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.inflight, 1.5, "test 24: option inflight 1.5  does not work")

    def test_prefetch_10(self):
        opt1 = "prefetch 10"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.prefetch, 10, "test 25: prefetch option did not work")
    ...
    def test_header_add(self):
        ...
        self.cfg.option(opt2.split())
        self.assertIn('toto1', self.cfg.headers_to_add, "test 26a: option header adding entries did not work")
        self.assertIn('toto2', self.cfg.headers_to_add, "test 26b: option header adding entries did not work")
        self.assertEqual(self.cfg.headers_to_add['toto1'], 'titi1',
                         "test 26c: option header adding entries did not work")
        self.assertEqual(len(self.cfg.headers_to_add), 2, "test 26d: option header adding entries did not work")
    ...
    def test_header_delete(self):
        ...
        self.assertIn('tutu1', self.cfg.headers_to_del, "test 27a: option header deleting entries did not work")
        self.assertIn('tutu2', self.cfg.headers_to_del, "test 27b: option header deleting entries did not work")
        self.assertEqual(len(self.cfg.headers_to_del), 2, "test 27c: option header deleting entries did not work")

    def test_expire_from_str(self):
        # expire in ms
        opt4 = "expire 10m"
        self.cfg.option(opt4.split())
        self.assertEqual(self.cfg.expire, 600000, "test 28: option expire or module duration_from_str did not work")

    def test_message_ttl(self):
        # message_ttl in ms
        opt4 = "message_ttl 20m"
        self.cfg.option(opt4.split())
        self.assertEqual(self.cfg.message_ttl, 1200000,
    ...
    def test_currentDir(self):
        ...
        self.cfg.option(opt4.split())
        self.assertNotIn('$', self.cfg.currentDir,
                         "test 30a: env variable substitution failed {}".format(self.cfg.currentDir))
        self.assertEqual(self.cfg.currentDir, 'michel/peter/jun/blabla',
                         "test 30b: env variable substitution failed {}".format(self.cfg.currentDir))

    def test_strip(self):
        opt4 = 'strip 4'
        self.cfg.option(opt4.split())
        self.assertEqual(self.cfg.strip, 4, "test 31: option strip with integer failed")
    ...
    def test_use_pika_false(self):
        opt4 = 'use_pika False'
        self.cfg.option(opt4.split())
        self.assertTrue(not self.cfg.use_pika and not self.cfg.use_amqplib,
                        "test 34: option use_pika boolean set to false failed")

    def test_statehost_false(self):
        opt4 = 'statehost False'
        self.cfg.option(opt4.split())
        self.assertFalse(self.cfg.statehost, "test 35: option statehost boolean set to false failed")

    def test_statehost_true(self):
        opt4 = 'statehost True'
        self.cfg.option(opt4.split())
        self.assertTrue(self.cfg.statehost, "test 36a: option statehost boolean set to true, hostform short, failed")
        self.assertEqual(self.cfg.hostform, 'short',
                         "test 36b: option statehost boolean set to true, hostform short, failed")

    def test_statehost_short(self):
        opt4 = 'statehost SHORT'
        self.cfg.option(opt4.split())
        self.assertTrue(self.cfg.statehost, "test 37a: option statehost set to SHORT, hostform short, failed")
        self.assertEqual(self.cfg.hostform, 'short', "test 37b: option statehost set to SHORT, hostform short, failed")

    def test_statehost_fqdn(self):
        opt4 = 'statehost fqdn'
        self.cfg.option(opt4.split())
        self.assertTrue(self.cfg.statehost, "test 38a: option statehost set to fqdn, hostform fqdn, failed")
        self.assertEqual(self.cfg.hostform, 'fqdn', "test 38b: option statehost set to fqdn, hostform fqdn, failed")

    def test_statehost_bad(self):
        opt4 = 'statehost TOTO'
        self.cfg.option(opt4.split())
        self.assertFalse(self.cfg.statehost, "test 39a: option statehost set badly ... did not react correctly, failed")
        self.assertIsNotNone(self.cfg.hostform,
    ...
    def test_extended(self):
        ...
        self.assertTrue(self.cfg.check_extended(),
    ...
    def test_base_dir(self):
        ...
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.base_dir, '/home/aspymjg/dev/metpx-sarracenia/sarra',
                         "test 44: string option base_dir did not work")

    def test_post_base_dir(self):
        opt1 = "post_base_dir /totot/toto"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.post_base_dir, '/totot/toto', "test 45: string option post_base_dir did not work")

    def test_post_base_url(self):
        opt1 = "post_base_url file://toto"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.post_base_url, 'file://toto', "test 46: url option post_base_url did not work")

    def test_outlet(self):
        self.assertEqual(self.cfg.outlet, 'post', "test 47: default error outlet = %s" % self.cfg.outlet)

    def test_outlet_json(self):
        opt1 = "outlet json"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.outlet, 'json', "test 48: option outlet value json did not work")

    def test_outlet_url(self):
        opt1 = "outlet url"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.outlet, 'url', "test 49: option outlet value url did not work")
    ...
    def test_outlet_post(self):
        opt1 = "outlet toto"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.outlet, 'post', "test 51: option outlet with bad value did not work")

    def test_retry_mode(self):
        self.assertTrue(self.cfg.retry_mode, "test 52: retry_mode should be the default")

    def test_retry_mode_false(self):
        opt1 = "retry_mode false"
        self.cfg.option(opt1.split())
        self.assertFalse(self.cfg.retry_mode, "test 53: retry_mode should be false")

    def test_retry_ttl_to_mins(self):
        opt1 = "retry_ttl 1D"
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.retry_ttl, 86400,
    ...
    def test_exchange_suffix(self):
        ...
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.exchange_suffix, 'suffix1',
                         "test 55a: option exchange_suffix or post_exchange_suffix did not work")
        self.assertEqual(self.cfg.post_exchange_suffix, 'suffix2',
    ...
    def test_broker(self):
        ...
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.post_base_dir, '/testbroker.toto/michel',
    ...
    def test_post_base_dir_replace(self):
        ...
        self.cfg.option(opt1.split())
        self.assertEqual(self.cfg.post_base_dir, '/tutu2/michel/aaa',
    ...
    def test_sundew_dirPattern(self):
        ...
        new_dir = self.cfg.sundew_dirPattern(urlstr='', basename='SACN04_CWAO_140251_RRA', destDir=d_dir,
                                             destName='aaa')
        self.assertTrue(new_dir.endswith('/SACN04CWAO140251RRA'),
                        "test 59: sundew_dirPattern new_dir %s should end with /SACN04CWAO140251RRA" % new_dir)
